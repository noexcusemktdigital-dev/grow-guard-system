...
import fixtures from '../fixtures/generate-social-concepts-inputs.json' assert { type: 'json' };

async function callFn(input: any): Promise<string> {
  const url = `${Deno.env.get('SUPABASE_URL')}/functions/v1/generate-social-concepts`;
  ...
  return data.message ?? data.content ?? data.concepts ?? JSON.stringify(data);
}

const suite: EvalSuite<any> = {
  name: 'generate-social-concepts',
  fnTarget: callFn,
