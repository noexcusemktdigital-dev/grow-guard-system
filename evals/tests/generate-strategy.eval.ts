...
import fixtures from '../fixtures/generate-strategy-inputs.json' assert { type: 'json' };

async function callFn(input: any): Promise<string> {
  const url = `${Deno.env.get('SUPABASE_URL')}/functions/v1/generate-strategy`;
...
const suite: EvalSuite<any> = {
  ...
  cases: fixtures.map((f: any) => ({
    ...
    matchers: [
      ...
      contains(f.input.business_type),
      contains(f.input.stage),
    ],
  })),
};
