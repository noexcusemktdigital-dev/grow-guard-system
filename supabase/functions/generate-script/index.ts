...
serve(async (req) => {
  ...
  try {
    ...
    // Build the prompt
    const contextBlock = `
CONTEXTO DO NEGÓCIO (extraído automaticamente):
- Segmento: ${context?.segment || "Não informado"}
- Produtos/Serviços: ${context?.products?.length ? context.products.map((p: any) => `${p.name} (R$${p.price})`).join(", ") : "Não cadastrados"}
...
- Retorne APENAS o script melhorado, sem comentários`;
    } else {
...
BRIEFING DO USUÁRIO:
${briefingBlock || "Nenhum briefing adicional fornecido."}
...
- Adapte o tom e linguagem ao segmento do negócio`;
          ...
          messages: [
            {
              role: "system",
              content:
                "Você é um consultor de vendas sênior brasileiro. Responda sempre em português brasileiro. Seja direto e prático.",
            },
    ...
    const suggestedTitle = mode === "improve"
      ? undefined
      : `Script de ${stageLabels[stage]} - ${context?.segment || "Geral"}`;

    return new Response(
      JSON.stringify({
        content,
        title: suggestedTitle,
        tags: [stageLabels[stage], context?.segment].filter(Boolean),
        tokens_used: tokensUsed,
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
