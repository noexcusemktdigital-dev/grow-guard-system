...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.user_id) {
      return await handleLegacyProvisioning(supabaseAdmin, body, headers, supabaseUrl);
    }
...
async function handleLegacyProvisioning(
  ...
  // (skipping auth header check since verify_jwt = false and this is called post-signup)
