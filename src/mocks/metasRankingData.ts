...
// ── EXTRA FRANCHISES (for more interesting ranking) ──

const extraFranchises: Franqueado[] = [
  { id: "f3", nomeUnidade: "Franquia Curitiba", status: "Ativo", percentualRepasse: 20, mensalidadeSistema: 250, observacoes: "Terceira franquia" },
  { id: "f4", nomeUnidade: "Franquia Ribeirão Preto", status: "Ativo", percentualRepasse: 20, mensalidadeSistema: 250, observacoes: "Quarta franquia" },
];

export function getAllFranchises(): Franqueado[] {
  return [...franqueados, ...extraFranchises];
}
...
export const mockCampaigns: Campaign[] = [
  {
    id: "camp1", name: "Desafio Fevereiro – Foco Assessoria", periodStart: "2026-02-01", periodEnd: "2026-02-28",
    goalType: "revenue", targetValue: 50000, rewardDescription: "1º lugar: R$2.000 + destaque na rede. 2º: desconto 50% royalties. 3º: curso premium gratuito.",
    status: "active",
  },
  {
    id: "camp2", name: "Sprint Janeiro – Novos Contratos", periodStart: "2026-01-01", periodEnd: "2026-01-31",
    goalType: "contracts", targetValue: 8, rewardDescription: "Troféu virtual + menção no comunicado mensal",
    status: "finished", winnerId: "f1", winnerName: "Franquia São Paulo Centro",
  },
];

// ── FRANCHISE REVENUE DATA (simulated per franchise) ──
...
export function calculatePoints(franchiseId: string, month: string): number {
  ...
  // Check if revenue goal is met (simple: target/numFranchises)
...
export function getNetworkTotals(month: string) {
  ...
  // Also pull real data from financial module
  ...
  return {
    ...
    targetRevenue,
    goalPercent,
    financialRevenue: summary.receitaBruta,
    receitaPorProduto: summary.receitaPorProduto,
    ranking,
  };
}
...
export function getEvolutionData(franchiseId?: string) {
  ...
  return months.map(m => {
    ...
    franchises.forEach(f => {
      entry[f.nomeUnidade] = calculateFranchiseRevenue(f.id, m.key);
    });
...
export function getRevenueByProduct(month: string) {
  const summary = getMonthSummary(month);
  const products = summary.receitaPorProduto;
