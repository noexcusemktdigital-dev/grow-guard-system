  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const opt = {
        ...
        filename: `proposta-noexcuse-${clientName || 'cliente'}-${new Date().toISOString().split('T')[0]}.pdf`,
        image: { type: 'jpeg' as const, quality: 0.98 },
        html2canvas: { scale: 2, useCORS: true, logging: false, backgroundColor: '#ffffff', scrollY: -window.scrollY },
        jsPDF: { unit: 'mm' as const, format: 'a4' as const, orientation: 'portrait' as const },
        pagebreak: { mode: ['css', 'legacy'], avoid: ['tr', 'img'] },
      };

      await html2pdf().set(opt).from(proposalRef.current).save();
    } catch (error) {
