...
import { getEventColor, getStatusColor } from "@/types/agenda";
import {
  format, isSameDay, isSameMonth, parseISO, startOfWeek, endOfWeek,
  eachDayOfInterval, startOfMonth, endOfMonth, isWithinInterval
} from "date-fns";
...
export function AgendaCalendar({ view, currentDate, activeCalendars, events, calendars, timeBlocks, onSelectEvent, onSelectDate, onCreateEvent }: Props) {
  if (view === "month") return <MonthView currentDate={currentDate} activeCalendars={activeCalendars} events={events} calendars={calendars} onSelectEvent={onSelectEvent} onSelectDate={onSelectDate} onCreateEvent={onCreateEvent} />;
  if (view === "week") return <WeekView currentDate={currentDate} activeCalendars={activeCalendars} events={events} calendars={calendars} timeBlocks={timeBlocks} onSelectEvent={onSelectEvent} onCreateEvent={onCreateEvent} />;
  return <DayView currentDate={currentDate} activeCalendars={activeCalendars} events={events} calendars={calendars} timeBlocks={timeBlocks} onSelectEvent={onSelectEvent} onCreateEvent={onCreateEvent} />;
}

// ===== helpers =====
function getEventsForDate(events: AgendaEvent[], date: Date, calendarIds: string[]): AgendaEvent[] {
  return events.filter(e => calendarIds.includes(e.calendarId) && isSameDay(parseISO(e.inicio), date));
}
...
// ===== MONTH =====
function MonthView({ currentDate, activeCalendars, events, calendars, onSelectEvent, onSelectDate, onCreateEvent }: Omit<Props, "view" | "timeBlocks">) {
  ...
  const monthEvents = useMemo(() => events.filter(e => {
    ...
    return activeCalendars.includes(e.calendarId) && isSameMonth(d, currentDate);
  }), [events, currentDate, activeCalendars]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-7 flex-1 auto-rows-fr">
        {days.map((day, i) => {
          const dayEvents = monthEvents.filter(e => isSameDay(parseISO(e.inicio), day));
          const isToday = isSameDay(day, new Date());
          const isCurrentMonth = isSameMonth(day, currentDate);
          return (
            <div
              key={i}
              className={`border-b border-r border-border p-1 min-h-[90px] cursor-pointer hover:bg-secondary/30 transition-colors ${!isCurrentMonth ? "bg-muted/20" : ""}`}
              onClick={() => onCreateEvent(day)}
            >
              <div className={`text-xs mb-1 w-6 h-6 flex items-center justify-center rounded-full ${isToday ? "bg-primary text-primary-foreground font-bold" : isCurrentMonth ? "text-foreground" : "text-muted-foreground/50"}`}>
                {format(day, "d")}
              </div>
                  ...
                  <Tooltip key={ev.id}>
                    <TooltipTrigger asChild>
                      <div
                        ...
                        style={{ backgroundColor: getEventColor(ev.calendarId, calendars) }}
...
// ===== WEEK =====
function WeekView({ currentDate, activeCalendars, events, calendars, timeBlocks, onSelectEvent, onCreateEvent }: Omit<Props, "view" | "onSelectDate">) {
  ...
  return (
    <div className="flex flex-col h-full overflow-auto">
      ...
      {(() => {
        const allDayEvents = days.map(d => getEventsForDate(events, d, activeCalendars).filter(e => e.allDay));
      ...
      <div className="flex-1">
        {HOURS.map(hour => (
          <div key={hour} className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-border/50 h-14">
            ...
            {days.map((d, di) => {
              const dayEvents = getEventsForDate(events, d, activeCalendars).filter(e => {
                if (e.allDay) return false;
                return parseISO(e.inicio).getHours() === hour;
              });
...
// ===== DAY =====
function DayView({ currentDate, activeCalendars, events, calendars, timeBlocks, onSelectEvent, onCreateEvent }: Omit<Props, "view" | "onSelectDate">) {
  const dayEvents = useMemo(() => getEventsForDate(events, currentDate, activeCalendars), [events, currentDate, activeCalendars]);
