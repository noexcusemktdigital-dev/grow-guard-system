...
export function CrmIntegrations() {
  ...
  const webhookUrl = orgId
    ? `https://${projectId}.supabase.co/functions/v1/crm-lead-webhook/${orgId}`
    : "Carregando...";
  ...
  const handleImport = async () => {
    ...
    setCsvData([]);
    setImporting(false);
    toast({ title: `Importação concluída: ${success} leads criados, ${errors} erros` });
  };
  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm flex items-center gap-2"><Plug className="w-4 h-4" /> Webhook de Leads</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Configure esta URL em suas plataformas (Meta Leads, Google Ads, formulários, etc.) para receber leads automaticamente.
          </p>
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            <p className="font-medium">Payload aceito (POST JSON):</p>
...
{`{
  ...
  "custom_fields": {}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm flex items-center gap-2"><FileSpreadsheet className="w-4 h-4" /> Importar CSV</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
