...
import {
  ...
  useAcademyQuizzes, useAcademyMutations, computeModuleProgress,
...
export function AcademyLesson({ lessonId, onBack, onNavigate, onGoToQuiz }: Props) {
  ...
  const { markLessonComplete } = useAcademyMutations();

  const lesson = allLessons.find(l => l.id === lessonId);
  if (!lesson) return null;

  const mod = modules.find(m => m.id === lesson.module_id);
  const lessons = allLessons.filter(l => l.module_id === lesson.module_id).sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
  const currentIndex = lessons.findIndex(l => l.id === lessonId);
  const prev = currentIndex > 0 ? lessons[currentIndex - 1] : null;
  const next = currentIndex < lessons.length - 1 ? lessons[currentIndex + 1] : null;
  const isCompleted = progress.some(p => p.lesson_id === lessonId && p.completed_at);
  const moduleProgress = computeModuleProgress(lesson.module_id, allLessons, progress);
  const allComplete = moduleProgress === 100;

  const handleComplete = () => {
    markLessonComplete.mutate(lessonId, {
      onSuccess: () => toast({ title: "Aula concluída!", description: `${lesson.title} marcada como concluída.` }),
    });
  };
      ...
      <div className="flex flex-col lg:flex-row gap-5">
          ...
          <Card className="p-5 space-y-3">
            ...
            <Button
              ...
              className={`w-full gap-2 text-base font-semibold transition-all ${
                isCompleted ? "" : "bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-500/25"
              }`}
              variant={isCompleted ? "outline" : "default"}
              disabled={isCompleted || markLessonComplete.isPending}
              onClick={handleComplete}
            >
              <CheckCircle2 className="w-5 h-5" />
              {isCompleted ? "✓ Concluída" : markLessonComplete.isPending ? "Salvando..." : "Marcar como Concluída"}
            </Button>
          </Card>
          ...
          <div className="flex items-center justify-between pt-2 gap-3">
            ...
            {next ? (
              ...
              <Button size="sm" className="gap-1" onClick={() => {
                const quiz = quizzes.find(q => q.module_id === lesson.module_id);
        ...
        <div className="w-full lg:w-72 flex-shrink-0">
          <Card className="p-3 space-y-1 sticky top-4">
            ...
            {lessons.map((l, i) => {
              const completed = progress.some(p => p.lesson_id === l.id && p.completed_at);
              const isCurrent = l.id === lessonId;
              return (
                <button
                  key={l.id}
                  className={`w-full text-left flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-colors text-sm ${
                    isCurrent ? "bg-primary/10 text-primary font-medium" : "hover:bg-muted/50 text-muted-foreground"
                  }`}
                  onClick={() => onNavigate(l.id)}
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold ${
                    completed ? "bg-emerald-500/15 text-emerald-600" : isCurrent ? "bg-primary/20 text-primary" : "bg-muted"
                  }`}>
