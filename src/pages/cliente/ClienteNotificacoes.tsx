...
export default function ClienteNotificacoes() {
  ...
  const filtered = filter === "Todos" ? allNotifs : allNotifs.filter(n => n.type === filter);
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map(n => {
            ...
            const timeAgo = n.created_at ? new Date(n.created_at).toLocaleDateString("pt-BR") : "";
            return (
              <Card
                key={n.id}
                className={`transition-all duration-200 cursor-pointer hover:shadow-md ${!n.is_read ? "border-primary/20" : "opacity-70"}`}
