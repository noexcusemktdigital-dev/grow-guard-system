import { Link2, Wifi, WifiOff, Settings2, RefreshCw, Unplug, AlertTriangle, Headset, Tag } from "lucide-react";
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <h3 className="text-sm font-semibold">
                                {instance.label || (instance.phone_number ? instance.phone_number : `Instância ${instance.instance_id.slice(0, 8)}...`)}
                              </h3>
                              <Badge
                                variant="outline"
                            ...
                            {instance.label && instance.phone_number && (
                              <div className="flex items-center gap-1 mt-1">
                                <Tag className="w-3 h-3 text-muted-foreground" />
                                <span className="text-[10px] text-muted-foreground">{instance.label}</span>
                              </div>
                            )}
                          </div>
                        </div>
                          ...
                          {isConnected ? (
                            <>
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleCheckStatus(instance.instance_id, instance.token, instance.client_token)}
                              ...
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleDisconnect(instance.instance_id, instance.token, instance.client_token)}
                                disabled={setupMutation.isPending}
              ...
              <CardContent className="p-4 flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-xs font-semibold text-amber-600 dark:text-amber-400">
                    Conta Z-API em modo trial?
                  </p>
                  <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                    Se suas mensagens estão chegando com o aviso de "CONTA EM TRIAL", ative um plano pago na Z-API para removê-lo.
                  </p>
                </div>
                <Button variant="outline" size="sm" onClick={() => window.open("https://app.z-api.io", "_blank")} className="shrink-0 text-xs">
                  Ver planos
                </Button>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardContent className="p-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold">Precisa de ajuda na implantação?</p>
                  <p className="text-xs text-muted-foreground">Nossa equipe pode te ajudar a configurar tudo</p>
                </div>
              </div>
              <Button variant="outline" size="sm" onClick={() => window.open(SUPPORT_LINK, "_blank")}>
                Falar com suporte
              </Button>
            </CardContent>
          </Card>
