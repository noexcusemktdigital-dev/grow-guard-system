...
import {
  ...
  Smile, Paperclip, Info, ChevronRight, Clock, MessageSquare, CheckCheck, Zap,
} from "lucide-react";
...
export default function ClienteChat() {
  ...
  const [mobileView, setMobileView] = useState<"accounts" | "conversations" | "chat">("conversations");
  ...
  const AccountsColumn = () => (
    <div className="flex flex-col h-full border-r border-border bg-card">
      <div className="p-3 border-b border-border">
        <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Contas WhatsApp</span>
      </div>
      <ScrollArea className="flex-1">
        <div className="p-2 space-y-1">
          <button
            onClick={() => { setSelectedAccount("all"); if (isMobile) setMobileView("conversations"); }}
            className={`w-full text-left p-3 rounded-xl transition-colors text-sm ${selectedAccount === "all" ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/30"}`}
          >
          ...
          {accounts.map(a => (
            <button
              key={a.id}
              onClick={() => { setSelectedAccount(a.id); if (isMobile) setMobileView("conversations"); }}
              className={`w-full text-left p-3 rounded-xl transition-colors ${selectedAccount === a.id ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/30"}`}
            >
              <div className="flex items-center gap-2">
                {a.status === "connected"
              ...
              {a.agentType && (
                <div className="ml-5.5 mt-1">
                  <Badge variant="outline" className="text-[9px] bg-purple-500/10 text-purple-400 border-purple-500/30">
                    <Bot className="w-2.5 h-2.5 mr-0.5" />{a.agentType}
                  </Badge>
                </div>
              )}
            </button>
      ...
      <div className="p-3 border-t border-border">
        <Button variant="outline" size="sm" className="w-full text-xs gap-1" onClick={() => setShowConnectDialog(true)}>
    ...
    <div className="flex flex-col h-full border-r border-border bg-card">
      <div className="p-3 space-y-2 border-b border-border">
        <div className="relative">
          ...
          <Input placeholder="Buscar conversa..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 h-9 text-sm" />
        </div>
        ...
        <div className="p-2 space-y-0.5">
          ...
          {filteredConversations.map(c => (
            <button
              ...
              className={`w-full text-left p-3 rounded-xl transition-colors ${c.id === selectedConversation ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/30"}`}
            >
              <div className="flex items-start gap-2.5">
                ...
                {c.unread > 0 && (
                  <span className="w-5 h-5 rounded-full bg-destructive text-destructive-foreground text-[10px] flex items-center justify-center font-bold flex-shrink-0 mt-1">
                    {c.unread}
                  </span>
                )}
              </div>
    ...
    return (
      <div className="flex-1 flex flex-col bg-card overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-border flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[11px] text-muted-foreground">{active.contactPhone}</span>
              ...
              {active.attendanceStatus === "humano" && active.attendant && (
                <span className="text-[10px] text-emerald-400 flex items-center gap-0.5">
                  <User className="w-3 h-3" /> {active.attendant}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center gap-1">
              {active.attendanceStatus === "ia" && (
                <Tooltip><TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleStatusChange(active.id, "humano")}>
                    <User className="w-4 h-4" />
                  </Button>
                </TooltipTrigger><TooltipContent>Assumir conversa</TooltipContent></Tooltip>
              )}
              {active.attendanceStatus === "humano" && (
                <Tooltip><TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleStatusChange(active.id, "ia")}>
              ...
              {active.attendanceStatus !== "encerrado" && (
                <>
                  ...
                  <Tooltip><TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleStatusChange(active.id, "encerrado")}>
              ...
              {active.crmLinked ? (
                ...
                </TooltipTrigger><TooltipContent>Vincular ao CRM</TooltipContent></Tooltip>
              )}
              <Tooltip><TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setShowContactPanel(!showContactPanel)}>
                  <Info className="w-4 h-4" />
                </Button>
              </TooltipTrigger><TooltipContent>Info do contato</TooltipContent></Tooltip>
            </div>
          </TooltipProvider>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden">
              ...
              {active.messages.map(m => {
                ...
                return (
                  <div key={m.id} className={`flex gap-2 ${isUser ? "flex-row-reverse" : ""}`}>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${isIA ? "bg-purple-500/20 text-purple-400" : "bg-muted"}`}>
                      {isIA ? <Bot className="w-3.5 h-3.5" /> : (m.avatar || (isUser ? "V" : "?"))}
                    </div>
          ...
          {/* Contact Panel */}
          {showContactPanel && active && (
            <div className="w-64 border-l border-border p-4 space-y-4 bg-card overflow-y-auto">
              <h4 className="text-sm font-semibold">Informações do Contato</h4>
              <div className="space-y-3">
                <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center text-lg font-bold mx-auto">
                  {active.avatar}
                </div>
                <div className="text-center">
                  <p className="text-sm font-semibold">{active.contactName}</p>
                  <p className="text-xs text-muted-foreground">{tagBadge(active.tag)}</p>
                </div>
                <div className="space-y-2 text-xs">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Phone className="w-3 h-3" /> {active.contactPhone}
                  </div>
                  {active.contactEmail && (
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <Mail className="w-3 h-3" /> {active.contactEmail}
                    </div>
                  )}
                </div>
              </div>
              {active.crmLinked ? (
                <div className="space-y-2 pt-2 border-t border-border">
                  <h5 className="text-xs font-semibold text-muted-foreground uppercase">CRM</h5>
        ...
        {active.attendanceStatus !== "encerrado" && (
          <div className="p-3 border-t border-border flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-9 w-9 flex-shrink-0 text-muted-foreground"><Smile className="w-4 h-4" /></Button>
            <Button variant="ghost" size="icon" className="h-9 w-9 flex-shrink-0 text-muted-foreground"><Paperclip className="w-4 h-4" /></Button>
            <Input
  ...
  if (isMobile) {
    return (
      <div className="h-[calc(100vh-120px)] flex flex-col">
        ...
        {mobileView === "accounts" && <AccountsColumn />}
        {mobileView === "conversations" && (
          <div className="flex-1 flex flex-col">
            <div className="px-3 pb-2 flex gap-2">
              <Button variant="outline" size="sm" className="text-xs" onClick={() => setMobileView("accounts")}>
                <Wifi className="w-3 h-3 mr-1" /> Contas
              </Button>
            </div>
            <ConversationsColumn />
          </div>
        )}
        {mobileView === "chat" && <ActiveChatColumn />}
        <Dialogs />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 h-[calc(100vh-120px)] flex flex-col">
      ...
      <div className="grid grid-cols-4 gap-3 flex-shrink-0">
        ...
        <KpiCard label="Tempo Médio Resposta" value="2m 45s" icon={Clock} />
        <KpiCard label="Fechadas Hoje" value={String(closedCount)} icon={CheckCheck} />
        <KpiCard label="Taxa de Resposta" value="94%" icon={Zap} trend="up" />
      </div>

      {/* 3-Column Layout */}
      <div className="flex-1 flex rounded-xl border border-border overflow-hidden bg-card">
        <div className="w-[200px] flex-shrink-0"><AccountsColumn /></div>
        <div className="w-[300px] flex-shrink-0"><ConversationsColumn /></div>
        <ActiveChatColumn />
      </div>
            ...
            <div className="space-y-4 py-4">
              <div className="flex items-center gap-2 p-4 rounded-xl bg-muted/50 border border-border">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Conecte via WhatsApp Business API, Z-API ou 360Dialog.</p>
                </div>
              </div>
              <Badge variant="outline" className="text-[10px]">Disponível no plano Pro</Badge>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showTransferDialog} onOpenChange={setShowTransferDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Selecione o atendente para transferir.</DialogDescription>
            </DialogHeader>
                      ...
                      setConversations(prev => prev.map(c =>
                        c.id === selectedConversation
                          ? {
                              ...
                              attendant: name,
                              messages: [...c.messages, { id: `s${Date.now()}`, sender: "system" as const, senderName: "Sistema", text: `Conversa transferida para ${name}`, time: new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) }],
                            }
                          : c
                      ));
                    }
