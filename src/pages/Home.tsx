...
import {
  ...
  ArrowUpRight, Target, Monitor, Bell,
} from "lucide-react";
...
export default function Home() {
  ...
  const smartAlerts = useMemo(() => {
    ...
    if (urgentTickets.length > 0) {
      items.push({ icon: Headphones, label: `${urgentTickets.length} chamado${urgentTickets.length > 1 ? "s" : ""} urgente${urgentTickets.length > 1 ? "s" : ""}`, detail: "Requerem atenção imediata", path: "/franqueadora/atendimento", variant: "destructive" });
    }

    const criticalAnn = (announcements ?? []).filter(a => a.priority === "Crítica");
    if (criticalAnn.length > 0) {
      items.push({ icon: Megaphone, label: `${criticalAnn.length} comunicado${criticalAnn.length > 1 ? "s" : ""} crítico${criticalAnn.length > 1 ? "s" : ""}`, detail: "Pendentes de confirmação da rede", path: "/franqueadora/comunicados", variant: "destructive" });
    }

    const sevenDaysAgo = new Date(Date.now() - 7 * 86400000).toISOString();
    const staleLeads = (leads ?? []).filter(l => !l.won_at && !l.lost_at && l.updated_at < sevenDaysAgo);
    if (staleLeads.length > 0) {
      items.push({ icon: Users, label: `${staleLeads.length} lead${staleLeads.length > 1 ? "s" : ""} sem atividade`, detail: "Há mais de 7 dias sem interação", path: "/franqueadora/crm", variant: "warning" });
    }
    ...
    <div className="w-full space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/15 via-primary/5 to-transparent border border-primary/10 p-6 lg:p-8">
        <div className="relative z-10">
          ...
          <p className="text-sm text-muted-foreground">Franqueadora · {hojeCapitalized}</p>
          {dailyMessage && (
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                  <Target className="w-4 h-4" /> Metas da Rede
                </h3>
              ...
              <Button variant="ghost" size="sm" className="mt-3 text-xs w-full" onClick={() => navigate("/franqueadora/metas")}>
                Ver todas as metas →
              </Button>
            </div>
